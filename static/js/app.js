...
async function fetchDashboards() {
    try {
        ...
        savedDashboardItems = Array.isArray(data.dashboards) ? data.dashboards : [];
...
async function saveDashboard() {
    ...
    try {
        ...
        savedDashboards.value = data.dashboard.id;
        updateSelectedDashboardPreview();
        showToast('Dashboard saved', 'success');
        setActionStatus(`Dashboard "${data.dashboard.name}" saved.`, 'success');
    } catch (error) {
...
async function deleteDashboard() {
    ...
    try {
        const response = await fetch(`/dashboards/${encodeURIComponent(selected.id)}`, {
            method: 'DELETE'
        });
        const data = await response.json();
        if (!response.ok || !data.success) {
